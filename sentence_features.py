...
def create_model(bert_config, is_training, input_ids_l, input_ids_r, input_mask_l, input_mask_r, segment_ids, ex_infos,
        ...
        output_weights_pos = tf.slice(output_weights, [1,0],[1,-1])   # get label=1
        output_weights_neg = tf.slice(output_weights, [0,0],[1,-1])
        tf.logging.info("  output_weights shape = %s" % ( output_weights.shape))
        tf.logging.info("  output_weights_pos shape = %s" % (output_weights_pos.shape))
        tf.logging.info("  output_weights_neg shape = %s" % (output_weights_pos.shape))
        tf.logging.info("  output_layer_l shape = %s" % (output_layer_l.shape))
        ...
        #return(output_layer_l, output_layer_l)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
	...
	def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
		...
		sentence_features_pos,sentence_features_neg = create_model(
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			output_spec = tf.contrib.tpu.TPUEstimatorSpec(
				mode=mode,
				predictions={"features_pos": sentence_features_pos,"features_neg": sentence_features_neg},
				scaffold_fn=scaffold_fn)
		return output_spec
...
def main(_):
	...
	output_predict_file_neg = os.path.join(FLAGS.output_dir, "sentence_features_neg.tsv")
	output_predict_file_pos = os.path.join(FLAGS.output_dir, "sentence_features_pos.tsv")	
	with tf.gfile.GFile(output_predict_file_neg, "w") as writer_neg, \
		tf.gfile.GFile(output_predict_file_pos, "w") as writer_pos:
		num_written_lines = 0
		tf.logging.info("***** Predict results *****")
		for i,prediction in enumerate(result):
			features_neg = prediction["features_neg"]
			features_pos = prediction["features_pos"]
			output_line_neg = "\t".join([str(i)]+
				[str(f)
				for f in features_neg]) + "\n"
			output_line_pos = "\t".join( [str(i)] +
				[str(f)
				for f in features_pos]) + "\n"

			writer_neg.write(output_line_neg)
			writer_pos.write(output_line_pos)
			num_written_lines += 1
