...
class DwProcessor(DataProcessor):
    """Processor for the Weibo data set ."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
                ...
                text_a = tokenization.convert_to_unicode(line[-2])
                #text_b = tokenization.convert_to_unicode(line[1])
                text_b = tokenization.convert_to_unicode(line[2]).split('@')[0] + ' '+ tokenization.convert_to_unicode(line[1])
                text_c = tokenization.convert_to_unicode(line[-3])
                text_d = tokenization.convert_to_unicode(line[-4])
                if set_type == "test":
                        label = [0 for i in range(FLAGS.num_variant_tags)]
                else:
...
#  tokens_a = tokenizer.tokenize(example.text_a)
    ...
    if example.text_d:
        embedding_ids = get_embedding_ids(example.text_d)
...
def file_based_convert_examples_to_features(
	...
	features["segment_ids"] = create_int_feature(feature.segment_ids)
        ...
        features["topic_ids"] = create_int_feature(feature.topic_ids)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, ex_infos, topic_ids,
    ...
    output_layer = tf.concat([output_layer, ex_infos], -1)

    prev_output = output_layer
    for layer_idx in range(FLAGS.num_exinfos_hidden_layers):
        cur_input = prev_output
        with tf.variable_scope("layer_%d" % layer_idx):
            with tf.variable_scope('ex_features'):
                hidden_size = cur_input.shape[-1].value
                prev_output = tf.layers.dense(
                    cur_input,
                    hidden_size,
                    activation=tf.tanh,
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
                prev_output = tf.nn.dropout(prev_output, keep_prob=0.6)

    output_layer = prev_output
    hidden_size = output_layer.shape[-1].value
