...
#WB multi-label classifier
class WBmultitagProcessor(DataProcessor):
	...
	def get_labels(self, data_dir):
		"""See base class."""
		for lines in self._read_tsv(os.path.join(data_dir, "labels.tsv")):
...
class Wb_binary_ab_Processor(DataProcessor):
	"""Processor for the Weibo data set ."""
...
class WideBinaryProcessor(DataProcessor):
	"""Processor for the Weibo data set ."""
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer):
	...
	tokens_b = []
	if example.text_b:
		fields = example.text_b.split(',')
		for i, phrase in enumerate(fields):
			tokens_b += tokenizer.tokenize(phrase)
			if (i + 1) != len(fields): tokens_b.append('[SEP]')
	...
	if tokens_b:
		...
		if FLAGS.exinfos_length != 0: limit = max_seq_length - 4
		if len(tokens_a) > limit:
			tokens_a = tokens_a[0:limit]
	...
	#reserved for wide feature position
	if FLAGS.exinfos_length != 0 and example.text_c:
		tokens.append("[SEP]")
		segment_ids.append(0)
		tokens.append("[SEP]")
		segment_ids.append(0)
	...
	for label in example.label:
		labels_ids.append(label_map[label])

	ex_infos = []
	if FLAGS.exinfos_length != 0 and example.text_c:
		for w in example.text_c.split(','): ex_infos.append(float(w))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, ex_infos,
	...
	with tf.variable_scope("loss"):
		...
		human_adjust = tf.logical_and(tf.greater(probabilities,0.04), tf.less(probabilities,0.96))
		per_example_loss = tf.cast(human_adjust,tf.float32) * per_example_loss
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
				...
				for j, logits in enumerate(logits_split):
					...
					logits = tf.cast(tf.math.round(logits), dtype=tf.int32)
					current_auc, update_op_auc = tf.metrics.accuracy(label_id_,logits)
					eval_dict['acc:'+str(j)] = (current_auc, update_op_auc)
...
def main(_):
	...
	processors = {
		...
		"wb_binary_ab": Wb_binary_ab_Processor,
		"wbmz": WbmzProcessor,
		"exinfos": ExinfosProcessor,
		"wide": WideProcessor,
		"widebinary": WideBinaryProcessor,
	}
